...
Labels are phantom types, and must be declared `sealed` (or `final`, or as an `object`). Shape operations such
as `swap`, `dropPrimes` or `jacobian` tell two labels apart with match types, which can only rule a case out once
the two types are provably distinct - an open trait could always be extended by a third type that is a subtype of
another label as well. `derives Label` rejects an open label with an explanatory error.
...
// Ensure that all axis types repl.MdocSession.MdocApp.UndefinedLabel are defined with 'derives Label' (e.g. 'sealed trait T derives Label')
// 
// val badShape = Shape(Axis[UndefinedLabel] -> 10)
//                                          ^
```
...
// Ensure that all axis types repl.MdocSession.MdocApp.NoLabel are defined with 'derives Label' (e.g. 'sealed trait T derives Label')
// 
// val noLabel = Tensor(Shape1(Axis[NoLabel] -> 2)).fill(1.0f)
//                                          ^
```
...
// Ensure that all axis types <error Not found: type A> are defined with 'derives Label' (e.g. 'sealed trait T derives Label')
// 
// error: 
// Not found: type B
// error: 
...
// Ensure that all axis types <error Not found: type B> are defined with 'derives Label' (e.g. 'sealed trait T derives Label')
// 
// error:
// Not found: type A
// def wrongFunc(x: Tensor2[A, B, Float32]): Tensor0[Float32] = x.sum
