...
trait FloatTensorTree[P]:
  def map(p: P, f: [T <: Tuple] => (Tensor[T, Float] => Tensor[T, Float])): P
  def zipMap(p1: P, p2: P, f: [T <: Tuple] => ((Tensor[T, Float], Tensor[T, Float]) => Tensor[T, Float])): P

object FloatTensorTree extends FloatTensorTreeLowPriority:
  def apply[P](using pt: FloatTensorTree[P]): FloatTensorTree[P] = pt

  given [Q <: Tuple]: FloatTensorTree[Tensor[Q, Float]] with
    def map(t: Tensor[Q, Float], f: [T <: Tuple] => (Tensor[T, Float] => Tensor[T, Float])): Tensor[Q, Float] =
      import TensorOps.retag
      f[Q](t.retag[Q])

    def zipMap(p1: Tensor[Q, Float], p2: Tensor[Q, Float], f: [T <: Tuple] => ((Tensor[T, Float], Tensor[T, Float]) => Tensor[T, Float])): Tensor[Q, Float] =
      import TensorOps.retag
      f[Q](p1.retag[Q], p2.retag[Q])
...
trait FloatTensorTreeLowPriority:
  given identity[A]: FloatTensorTree[A] = new FloatTensorTree[A]:
    def map(p: A, f: [T <: Tuple] => (Tensor[T, Float] => Tensor[T, Float])): A = p
    def zipMap(p1: A, p2: A, f: [T <: Tuple] => ((Tensor[T, Float], Tensor[T, Float]) => Tensor[T, Float])): A = p1
