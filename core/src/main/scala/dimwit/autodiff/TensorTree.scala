...
trait TensorTree[P]:
  def map(p: P, f: [T <: Tuple, V] => (Tensor[T, V] => Tensor[T, V])): P
  def zipMap(p1: P, p2: P, f: [T <: Tuple, V] => ((Tensor[T, V], Tensor[T, V]) => Tensor[T, V])): P

object TensorTree extends TensorTreeLowPriority:
  def apply[P](using pt: TensorTree[P]): TensorTree[P] = pt

  given [Q <: Tuple, V]: TensorTree[Tensor[Q, V]] with
    def map(t: Tensor[Q, V], f: [T <: Tuple, V2] => (Tensor[T, V2] => Tensor[T, V2])): Tensor[Q, V] =
      import TensorOps.retag
      f[Q, V](t.retag[Q])

    def zipMap(p1: Tensor[Q, V], p2: Tensor[Q, V], f: [T <: Tuple, V2] => ((Tensor[T, V2], Tensor[T, V2]) => Tensor[T, V2])): Tensor[Q, V] =
      import TensorOps.retag
      f[Q, V](p1.retag[Q], p2.retag[Q])
...
trait TensorTreeLowPriority:
  given identity[A]: TensorTree[A] = new TensorTree[A]:
    def map(p: A, f: [T <: Tuple, V] => (Tensor[T, V] => Tensor[T, V])): A = p
    def zipMap(p1: A, p2: A, f: [T <: Tuple, V] => ((Tensor[T, V], Tensor[T, V]) => Tensor[T, V])): A = p1
