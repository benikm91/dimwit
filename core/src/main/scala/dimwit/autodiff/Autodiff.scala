...
object Autodiff:
  ...
  /** The derivative of a function `In => Out`: the structure of `Out`, with every tensor in it
    * replaced by that tensor's derivative with respect to all of `In`.
    *
    * This is a type class rather than a match type because it has to look through case classes,
    * which needs their [[scala.deriving.Mirror]] - something a match type cannot ask for.
    */
  @implicitNotFound(
    "Cannot express the derivative of ${Out} with respect to ${In}. Both must be built from tensors, tuples, named tuples or case classes"
  )
  ...
  object Gradient extends GradientLowPriority:
    ...
    type Aux[In, Out, R] = Gradient[In, Out]:
      type Result = R
    ...
    /** An output tensor is differentiated against every tensor of the input. */
    ...
    given tuple[In, H, HR, T <: Tuple, TR <: Tuple](using
        head: Aux[In, H, HR],
        tail: Aux[In, T, TR]
  ...
  trait GradientLowPriority:
    /** A case class output becomes a named tuple of its field derivatives, keeping the field names.
      * Lower priority than the tuple cases, as tuples are Products too.
      */
  ...
  /** The derivative of a single output tensor of shape `OutShape` with respect to all of `In`.
    * Mirrors [[Gradient]], but recurses on the input structure.
    */
  ...
  object GradientTensorVsInput extends GradientTensorVsInputLowPriority:
    ...
    type Aux[In, OutShape <: Tuple, V, R] = GradientTensorVsInput[In, OutShape, V]:
      type Result = R
    ...
    /** The output axes come first, then the input axes, primed where the two collide. */
    given tensor[InShape <: Tuple, InV, OutShape <: Tuple, V]: Aux[Tensor[InShape, InV], OutShape, V, Tensor[PrimeConcat[OutShape, InShape], V]] = instance

    given emptyTuple[OutShape <: Tuple, V]: Aux[EmptyTuple, OutShape, V, EmptyTuple] = instance

    given tuple[H, HR, T <: Tuple, TR <: Tuple, OutShape <: Tuple, V](using
