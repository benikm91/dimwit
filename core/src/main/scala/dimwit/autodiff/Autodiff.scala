...
object Autodiff:
  ...
  def valueAndGrad[Input, V](f: Input => Tensor0[V])(using
    ...
    (params: Input) =>
      val xpy = inTree.toPyTree(params)
      val pygrad = gpy(xpy)
      val (value, grad) = (pygrad.bracketAccess(0), pygrad.bracketAccess(1))
      (outTree.fromPyTree(value).asInstanceOf[Tensor0[V]], Grad(inTree.fromPyTree(grad).asInstanceOf[Input]))
